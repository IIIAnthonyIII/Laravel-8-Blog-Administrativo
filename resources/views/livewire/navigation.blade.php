<nav class="bg-gray-800" x-data="{ open: false }">
    <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div class="relative flex items-center justify-between h-16">
                ...
                <div class="hidden sm:block sm:ml-6">
                    <div class="flex space-x-4">
                        ...
                        @foreach ($categoriesNav as $category)
                            <a href="{{route('posts.category', $category)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{$category->name_category}}</a>
                        @endforeach
                    </div>
                </div>
            </div>
            ...
            <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                ...
                <!-- Profile dropdown -->
                <!-- Al agregar x-data que tiene un objeto, se convierte en componente -->
                <div class="ml-3 relative" x-data="{ open: false }">
                    ...
                    <!-- Con el evento x-on:click.away cierro en cualquier parte de la pagina -->
    ...
    <div class="sm:hidden" id="mobile-menu" x-show="open" x-on:click.away="open=false">
        ...
        @foreach ($categoriesNav as $category)
            <a href="{{route('posts.category', $category)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$category->name_category}}</a>
        @endforeach
        </div>
    </div>
</nav>